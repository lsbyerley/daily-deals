...
export default function DealModule(props: DealModuleProps) {
  ...
  const NoDealsMessage = ({ msg }: { msg: string }) => {
    return (
      <div className='text-foreground text-center'>
        <p className='inline-flex items-center justify-center'>
          {msg} <Frown className='ml-2' />
        </p>
      </div>
    );
  };
  ...
  return (
    <div className='flex flex-col gap-14 animate-in'>
      <div className='flex flex-col gap-8 text-foreground'>
        ...
        {todayDeals.length == 0 && (
          <NoDealsMessage msg={`No ${today} deals found`} />
        )}
      </div>
      ...
      <div className='flex flex-col gap-8 text-foreground'>
        ...
        {happyHourDeals.length == 0 && (
          <NoDealsMessage msg={`No happy hour deals found`} />
        )}
      </div>
      ...
      <div className='flex flex-col gap-8 text-foreground'>
        ...
        {everydayDeals.length == 0 && (
          <NoDealsMessage msg={`No every day deals found`} />
        )}
      </div>
    </div>
  );
