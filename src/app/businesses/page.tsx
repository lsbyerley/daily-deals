...
export default async function Businesses({ searchParams }: BusinessesPageProps) {
  ...
  const { data: businesses } = await supabase.from('businesses').select().match({ 'city': city, state: region})
      ...
      <div className="animate-in flex flex-col gap-14 opacity-0 max-w-4xl px-3 py-16 lg:py-24 text-foreground">
        ...
        <div className="flex flex-col gap-8 text-foreground">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            {businesses?.map(({ id, name, street, city, state, zipcode, type, website }) => (
              <a
                key={name}
              ...
              >
                ...
                <div className="flex flex-col grow gap-4 justify-between">
                  <p className="text-sm opacity-70">{street}</p>
